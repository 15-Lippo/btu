import { Pair, Token, TokenAmount } from '@dexs/sdk'
import { CurrencyAmount } from '@dexs/sdk/dist/entities/fractions/currencyAmount'
...
export function usePairsComparator(inverted: boolean): (pairA: Pair, pairB: Pair) => number {
  ...
  const balances = useTokenBalances(
    account || undefined,
    trackedTokenPairs.map(rawPair => toDexSwapLiquidityToken(rawPair))
  )
