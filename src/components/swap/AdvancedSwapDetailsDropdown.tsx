...
import { ChainId, RoutablePlatform, Trade } from '@dexs/sdk'
...
const HideableAutoColumn = styled(AutoColumn)<{ show: boolean }>`
  transform: ${({ show }) => (show ? 'translateY(8px); opacity: 1' : 'translateY(-100%); opacity: 0')};
...
export default function AdvancedSwapDetailsDropdown({
  ...
  return (
    <HideableAutoColumn gap="8px" show={!!trade}>
      ...
      {chainId === ChainId.HARMONY && !!mainnetGasPrices && (
        <SettingsFlex>
          <Box flex="1">
