import { Currency, CurrencyAmount, Pair, RoutablePlatform, Token, Trade } from '@dexs/sdk'
...
export function useTradeExactInAllPlatforms(
  ...
  const bestTrades = [
    ...
    useTradeExactIn(currencyAmountIn, currencyOut, RoutablePlatform.VIPERSWAP)
  ]
...
export function useTradeExactOutAllPlatforms(
  ...
  const bestTrades = [
    ...
    useTradeExactOut(currencyIn, currencyAmountOut, RoutablePlatform.VIPERSWAP)
  ]
  return sortTradesByExecutionPrice(bestTrades).filter(trade => !!trade)
}
