import { CurrencyAmount, JSBI, Trade, Token, RoutablePlatform } from '@dexs/sdk'
import React, { useCallback, useContext, useEffect, useMemo, useState } from 'react'
import { Repeat } from 'react-feather'
import { Text } from 'rebass'
import styled, { ThemeContext } from 'styled-components'
import { ButtonError, ButtonPrimary, ButtonPrimary1, ButtonConfirmed } from '../../components/Button'
import Card from '../../components/Card'
import Column, { AutoColumn2, AutoColumn3 } from '../../components/Column'
...
export default function Swap() {
  ...
  return (
    <>
      ...
      />
      <AppBody1 tradeDetailsOpen={!!trade}>
        <SwapPoolTabs active={'swap'} />
        <Wrapper id="swap-page" className="none-transparent heighter">
          <ConfirmSwapModal
            isOpen={showConfirm}
            trade={trade}
          ...
          <AutoColumn2 gap="12px">
            ...
            {!showWrap && !!trade && (
              <AutoColumn3 gap="5px 10px" style={{border:"1px solid #ff360026"}}>
                <PaddedCard py="0px" px="8px">
                  <RowBetween alignItems="center">
                    ...
                    <QuestionHelper text="The trade is routed directly to the selected platform, so no swap or network fees are ever added by DexSwap." />
                  </RowBetween>
                </PaddedCard>
            ...
            <div>
              {!account ? (
                <ButtonPrimary1 onClick={toggleWalletSwitcherPopover}>Connect Wallet</ButtonPrimary1>
              ) : showWrap ? (
                <ButtonPrimary1 disabled={Boolean(wrapInputError)} onClick={onWrap}>
                  {wrapInputError ??
                    (wrapType === WrapType.WRAP ? 'Wrap' : wrapType === WrapType.UNWRAP ? 'Unwrap' : null)}
                </ButtonPrimary1>
              ) : noRoute && userHasSpecifiedInputOutput ? (
                <ButtonPrimary1 style={{ textAlign: 'center',  background: '#7f7f90' }} disabled>
