import { Pair, Percent, Token, TokenAmount } from '@dexs/sdk'
    ...
    createLiquidityMiningCallback()
      .then(transaction => {
        ...
        addTransaction(transaction, {
          summary: `Create Token Rewards Campaign on ${targetedPair?.token0.symbol}/${targetedPair?.token1.symbol}`
        })
      })
      .catch(error => {
        console.error(error)
  ...
  return (
    <>
      <PageWrapper gap="40px">
        <AutoColumn gap="8px">
          <TYPE.mediumHeader textAlign="center" lineHeight="24px">{t('liquidityMining.create.title')}</TYPE.mediumHeader>
          <TYPE.subHeader color="text5" lineHeight="17px" textAlign="center">
            {t('liquidityMining.create.subtitle')}
          </TYPE.subHeader>
        </AutoColumn>
        <Step title="Select pair and reward" index={0} disabled={singleReward === null}>
          <PairAndReward
        ...
        <Step title="Select reward amount" index={1} disabled={!targetedPair || !reward || !reward.token}>
          <RewardAmount
            reward={reward}
        ...
        <Step title="Duration and start/end time" index={2} disabled={!reward || reward.equalTo('0')}>
          <Time
            startTime={startTime}
            endTime={endTime}
